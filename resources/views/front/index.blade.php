...
<body class="font-[Poppins] pb-[72px]">
    ...
    <section id="Featured" class="mt-[30px]">
        <div class="main-carousel w-full">
            ...
            <div class="featured-news-card relative w-full h-[550px] flex shrink-0 overflow-hidden">
                <img src="{{Storage::url($featured_article->thumbnail)}}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
                ...
                <div class="card-detail max-w-[1130px] w-full mx-auto flex items-end justify-between pb-10 relative z-20">
                    <div class="flex flex-col gap-[10px]">
                        ...
                        <a href="{{route('front.details', $featured_article->slug)}}" class="font-bold text-4xl leading-[45px] text-white two-lines hover:underline transition-all duration-300">{{$featured_article->name}}</a>
                        <p class="text-white">{{$featured_article->created_at->format('M d, Y')}} • {{$featured_article->category->name}}</p>
                    </div>
            ...
            @empty
            <p>Belum ada article bagus nih, coba anda nulis</p>
            @endforelse
        </div>
    </section>
    ...
    <section id="Best-authors" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        ...
        <div class="grid grid-cols-6 gap-[30px]">
            ...
            @empty
            <p>Belum punya penulis, jadilah salah satunya</p>
            @endforelse
        </div>
    </section>
    ...
    <section id="Latest-entertainment" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        ...
        <div class="flex justify-between items-center h-fit">
            @if ($featured_entertainment != null)
            <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
                <img src="{{Storage::url($featured_entertainment->thumbnail)}}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
                <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
                <div class="card-detail w-full flex items-end p-[30px] relative z-20">
                    <div class="flex flex-col gap-[10px]">
                        <p class="text-white">Featured</p>
                        <a href="{{route('front.details', $featured_entertainment->slug)}}" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{substr($featured_entertainment->name, 0, 50)}}{{strlen($featured_entertainment->name) > 50 ? '...' : ''}}</a>
                        <p class="text-white">{{$featured_entertainment->created_at->format('M d, Y')}}</p>
                    </div>
                </div>
            </div>
            @else
            <p>Belum ada artikel ini</p>
            @endif
            <div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
                <div class="w-[455px] flex flex-col gap-5 shrink-0">
                    @forelse ($entertainment_articles as $entertainment_article)
                    <a href="{{route('front.details', $entertainment_article->slug)}}" class="card py-[2px]">
                        <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
                            <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                                <img src="{{Storage::url($entertainment_article->thumbnail)}}" class="object-cover w-full h-full" alt="thumbnail" />
                            </div>
                            <div class="flex flex-col justify-center-center gap-[6px]">
                                <h3 class="font-bold text-lg leading-[27px]">{{ substr($entertainment_article->name, 0, 50)}}{{strlen($entertainment_article->name) > 50 ? '...' : ''}}</h3>
                                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{$entertainment_article->created_at->format('M d, Y')}}</p>
                            </div>
                        </div>
                    </a>
                    @empty
                    <p>Belum ada article tentang ini.</p>
                    @endforelse
                </div>
    ...
    <section id="Latest-business" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        ...
        <div class="flex justify-between items-center h-fit">
            @if ($featured_automotive != null)
            <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
                <img src="{{Storage::url($featured_automotive->thumbnail)}}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
                <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
                <div class="card-detail w-full flex items-end p-[30px] relative z-20">
                    <div class="flex flex-col gap-[10px]">
                        <p class="text-white">Featured</p>
                        <a href="details.html" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{substr($featured_automotive->name, 0, 50)}}{{strlen($featured_automotive->name) > 50 ? '...' : ''}}</a>
                        <p class="text-white">{{$featured_automotive->created_at->format('M d, Y')}}</p>
                    </div>
                </div>
            </div>
            @else
            ...
            <div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
                <div class="w-[455px] flex flex-col gap-5 shrink-0">
                    @forelse ($automotive_articles as $automotive_article)
                    <a href="{{route('front.details', $automotive_article->slug)}}" class="card py-[2px]">
                        <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
                            <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                                <img src="{{Storage::url($automotive_article->thumbnail)}}" class="object-cover w-full h-full" alt="thumbnail" />
                            </div>
                            <div class="flex flex-col justify-center-center gap-[6px]">
                                <h3 class="font-bold text-lg leading-[27px]">{{substr($automotive_article->name, 0, 50)}}{{strlen($automotive_article->name) > 50 ? '...' : ''}}</h3>
                                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{$automotive_article->created_at->format('M d, Y')}}</p>
                            </div>
                        </div>
                    </a>
                    @empty
    ...
    <section id="Latest-automotive" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        <div class="flex justify-between items-center">
            ...
            <a href="{{route('front.category', 'health')}}" class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18]">Explore All</a>
        </div>
        <div class="flex justify-between items-center h-fit">
            @if ($featured_health != null)
            <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
                <img src="{{Storage::url($featured_health->thumbnail)}}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
                <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
                <div class="card-detail w-full flex items-end p-[30px] relative z-20">
                    <div class="flex flex-col gap-[10px]">
                        <p class="text-white">Featured</p>
                        <a href="details.html" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{substr($featured_health->name, 0, 50)}}{{strlen($featured_health->name) > 0 ? '...' : ''}}</a>
                        <p class="text-white">{{$featured_health->created_at->format('M d, Y')}}</p>
                    </div>
                </div>
            </div>
            @else
            ...
            <div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
                <div class="w-[455px] flex flex-col gap-5 shrink-0">
                    @forelse ($health_articles as $health_article)
                    <a href="{{route('front.details', $health_article->slug)}}" class="card py-[2px]">
                        <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
                            <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                                <img src="{{Storage::url($health_article->thumbnail)}}" class="object-cover w-full h-full" alt="thumbnail" />
                            </div>
                            <div class="flex flex-col justify-center-center gap-[6px]">
                                <h3 class="font-bold text-lg leading-[27px]">{{substr($health_article->name, 0, 50)}}{{strlen($health_article->name) > 50 ? '...' : ''}}</h3>
                                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{$health_article->created_at->format('M d, Y')}}</p>
                            </div>
                        </div>
                    </a>
                    @empty
